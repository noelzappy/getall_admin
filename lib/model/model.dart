...
class MainModel with ChangeNotifier, DiagnosticableTreeMixin {
    ...
    if (user != null) {
      ...
      FirebaseFirestore.instance
        ...
        if (querySnapshot.exists) {
          ...
          userAvatar =
              (data["logoServerPath"] != null) ? data["logoServerPath"] : "";
          _redraw();
        }
      });
    }
  ...
  setUserData(Uint8List? _imageData, String name) async {
    ...
    if (name.isEmpty) return strings.get(91);
    ...
    userName = name;
    if (serverPath.isNotEmpty) userAvatar = serverPath;
    return null;
  }

  //bool demo = false;
  // String appname = "GETALL";
  ...
  getTimeFormat() {
    if (appSettings.timeFormat == "12h") return 'hh:mm a';
    return 'HH:mm';
  }
  ...
  String getDateTimeString(DateTime _time) {
    return df.DateFormat(appSettings.dateFormat).format(_time).toString() +
        " " +
  ...
  double getWorkspaceWidth() {
    var d =
        (isMenuShow && windowWidth >= 1200) ? windowWidth - 300 : windowWidth;
    if (d < 800) d = 800;
    return d;
  }
  ...
  Future<String?> init(Function(String) _callback, BuildContext context,
      Function()? _redrawMenu) async {
    ...
    if (ret != null) return ret;
    _callback(strings.get(213));

    /// "Loading Service App theme ...",
    ret = await serviceApp.loadEmulatorServiceAppData();
    if (ret != null) return ret;
    _callback(strings.get(218));

    /// "Loading Category ...",
    ret = await category.load();
    if (ret != null) return ret;
  ...
  playSound() async {
    ...
    if (soundId != 0) await pool.play(soundId);
  }

  Future<String?> saveSettingsDocuments(
      String _copyright, String _about, String _policy, String _terms) async {
    return settings.saveSettingsDocuments(_copyright, _about, _policy, _terms);
  }
  ...
  List<ComboData> appsDataCombo = [
    ...
    ComboData(strings.get(389), "site"),
  ...
  String getTextByLocale(List<StringData> _data) {
    for (var item in _data)
      if (item.code == langEditDataComboValue) return item.text;
    for (var item in _data) if (item.code == "en") return item.text;
    return "";
  }
  ...
  setEmulatorServiceAppLang() async {
    for (var item in serviceAppLangs)
      if (item.locale == currentEmulatorLanguage) {
        if (item.data.isEmpty) await langs.loadLanguage(item);
  ...
  Future<String?> setImageData(Field item, Uint8List _imageData) async {
    try {
      ...
      var firebaseStorageRef = FirebaseStorage.instance.ref().child(name);
      TaskSnapshot s = await firebaseStorageRef.putData(_imageData);
      item.serverPath = await s.ref.getDownloadURL();
      item.localFile = name;
      item.value = false;
  ...
  copyReviews() {
    ...
    for (var item in reviews) {
      text = "$text${item.id}\t${item.rating}\t${item.userName}"
          "\t${item.serviceName}\t${getDateTimeString(item.time)}\t${item.text}"
          "\n";
    }
