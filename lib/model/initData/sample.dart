...
Future<String?> uploadSampleData(Function(String) callback, BuildContext context) async{
  ...
  try{
    ListResult result = await FirebaseStorage.instance.ref().child('category').listAll();
  ...
  try{
    var result = await FirebaseStorage.instance.ref().child('provider').listAll();
    for (var file in result.items){
      var reference = FirebaseStorage.instance.ref(file.fullPath);
      await reference.delete();
    }
  ...
  try{
    var result = await FirebaseStorage.instance.ref().child('service').listAll();
    for (var file in result.items){
      var reference = FirebaseStorage.instance.ref(file.fullPath);
      await reference.delete();
    }
...
_uploadProviderImage(ProviderData item, String type) async {
  ...
  TaskSnapshot s = await firebaseStorageRef.putData(_imageData);
  if (type == "assetUpperImage") {
    item.imageUpperServerPath = await s.ref.getDownloadURL();
    item.imageUpperLocalFile = name;
  }
  if (type == "assetsLogo") {
    item.logoServerPath = await s.ref.getDownloadURL();
...
_uploadProviderGalleryImage(ProviderData item) async {
  ...
  for (var item2 in item.assetsGallery) {
    ...
    var firebaseStorageRef = FirebaseStorage.instance.ref().child(name);
    TaskSnapshot s = await firebaseStorageRef.putData(_imageData);
    _gallery.add(ImageData(serverPath: await s.ref.getDownloadURL(), localFile: name));
  }
...
_uploadServiceGalleryImage(ProductData item) async {
  ...
  for (var item2 in item.assetsGallery) {
    ...
    var firebaseStorageRef = FirebaseStorage.instance.ref().child(name);
    TaskSnapshot s = await firebaseStorageRef.putData(_imageData);
    _service.add(ImageData(serverPath: await s.ref.getDownloadURL(), localFile: name));
  }
...
List<ProviderData> _provider = [
  ProviderData(
    ...
    tax: 9,
    route: [],
    addon: [], workTime: [], gallery: [], category: [],
  ),
  ProviderData(
