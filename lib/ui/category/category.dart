...
import 'package:ondemand_admin/ui/category/tree.dart';
...
class _CategoryScreenState extends State<CategoryScreen> {
  ...
  void dispose() {
    ...
    _mainModel.category.current = CategoryData.createEmpty();
    super.dispose();
  }
  ...
  _getList(){
    ...
    context.watch<MainModel>().category.current;
    ...
    if (_mainModel.category.ensureVisible.isNotEmpty){
      for (var item in _mainModel.category.category){
        ...
        _currentPage = 1;
        _visibleCount++;
        if (item.id == _mainModel.category.ensureVisible){
          var _pStart2 = 0;
          do{
            if (_visibleCount-1 > _pStart2 && _visibleCount-1 < _pStart2+_pRange) {
              _pStart = _pStart2;
              _mainModel.category.ensureVisible = "";
              break;
            }
            _pStart2+=_pRange;
            _currentPage++;
          }while(_pStart2<_mainModel.category.category.length);
        }
      }
    }
    ...
    for (var item in _mainModel.category.category){
      ...
      _cells.add(DataRow(cells: [
        ...
        // preview
        DataCell(Container(child: Text(_mainModel.getTextByLocale(item.desc),
            overflow: TextOverflow.ellipsis, style: theme.style14W400))),
        // action
    ...
    if (isMobile()){
      list.add(TreeInCategory(deleteDialog: _openDialogDelete));
      list.add(SizedBox(height: 10,));
      if (!_notEditorCreate)
        list.add(EditInCategory());
      else
        list.add(Center(child: button2small(strings.get(77), (){ /// "Create new category",
          _notEditorCreate = false;
          _redraw();
        })));
      list.add(SizedBox(height: 10,));
      ...
      list.add(Row(
          children: [
            ...
            Expanded(child: Column(
              children: [
                TreeInCategory(deleteDialog: _openDialogDelete),
                SizedBox(height: 20,),
                if (_notEditorCreate)
                  Center(child: button2small(strings.get(77), (){ /// "Create new category",
                    _notEditorCreate = false;
                    _redraw();
                  })),
                if (!_notEditorCreate)
  ...
  _openDialogDelete(CategoryData value){
    openDialogDelete(() async {
      ...
      var ret = await _mainModel.category.delete(value);
      if (ret == null)
