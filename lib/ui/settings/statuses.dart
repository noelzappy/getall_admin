...
class BookingStatusesScreen extends StatefulWidget {
  ...
  const BookingStatusesScreen({required this.waits});
...
class _BookingStatusesScreenState extends State<BookingStatusesScreen> {
  ...
  _getList() {
    ...
    list2.add(Center(child: button2small(strings.get(9), () async {
      // demo mode
      if (appSettings.demo)
    ...
    if (Provider.of<MainModel>(context,listen:false).currentStatus.id.isNotEmpty)
      list3.add(Row(
        children: [
          button2small(strings.get(225), (){}), /// "Save current",
          SizedBox(width: 10,),
          button2small(strings.get(226), (){  /// "Create new",
      ...
      list3.add(button2small(strings.get(174), (){  /// "Add new",
    ...
    list2.add(
        Row(
          children: [
            ...
            if (item.serverPath.isNotEmpty)
              SizedBox(width: 10,),
            Expanded(child: button2small(strings.get(75), /// "Select image",
                (){_selectImage(item);})),
          ],
        )
    );
