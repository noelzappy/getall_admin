...
class _ProvidersScreenState extends State<ProvidersScreen> {
    ...
    return Stack(
        children: [
          ...
          (darkMode) ? Colors.black : Colors.white,
              ...
              (_scroller > 5) ? "" : getTextByLocale(_mainModel.provider.current.name, _mainModel.currentEmulatorLanguage),
                  context, () {}),
  ...
  _title() {
    var imageUpperServerPath = _mainModel.provider.current.imageUpperServerPath;
    return Container(
  ...
  _body(){
    ...
    for (var item in _mainModel.category.category)
      if (item.select)
    ...
    list.add(Container(
        ...
        child: Card50forProvider(
          ...
          category: _mainModel.category.category,
          direction: strings.direction,
          provider: _mainModel.provider.current,
        ),
        ));
    ...
    List<Widget> list2 = [];
    if (_mainModel.provider.current.phone.isNotEmpty)
      list2.add(InkWell(
          onTap: () {
    ...
    if (_mainModel.provider.current.www.isNotEmpty)
      list2.add(InkWell(
          onTap: () {
            //openUrl(_shopinfo.website);
          }, // needed
    ...
    if (_mainModel.provider.current.instagram.isNotEmpty)
      list2.add(InkWell(
          onTap: () {
            //openUrl(_shopinfo.website);
          }, // needed
    ...
    if (_mainModel.provider.current.telegram.isNotEmpty)
      list2.add(InkWell(
          onTap: () {
            //openUrl(_shopinfo.website);
          }, // needed
    ...
    if (_mainModel.provider.current.descTitle.isNotEmpty)
      list.add(Container(
          ...
          child: Column(
            ...
            children: [
              Text(getTextByLocale(_mainModel.provider.current.descTitle, _mainModel.currentEmulatorLanguage),
                style: theme.style16W800, textAlign: TextAlign.start,),   /// "Description",
              Divider(color: (darkMode) ? Colors.white : Colors.black),
              SizedBox(height: 5,),
              Text(getTextByLocale(_mainModel.provider.current.desc, _mainModel.currentEmulatorLanguage), style: theme.style14W400),
              SizedBox(height: 5,),
              Divider(color: (darkMode) ? Colors.white : Colors.black),
            ],
          )
    ...
    var index = 0;
    for (var item in _mainModel.provider.current.workTime){
      if (item.weekend)
        continue;
      list.add(Container(
